...
module.exports = function(connect) {
   ...
   *
   * @param {(callback: (err?: Error, coll: import('mongodb').Collection) => void) => void} [getCollection] async function to return the sessions collection
   ...
   * @param {Object} [options.serialize] Flag to enable session serialization (default true)
   * @public
   */
  ...
  function Yams(getCollection, options) {
    ...
    // serialize by default should be true to provide backward compatibility
    this.options.serialize = this.options.serialize === false ? false : true;
    ...
    /**
     * 
     * @param {(collection: import('mongodb').Collection) => void} callback 
     * @returns 
     */
  ...
  Yams.prototype.serialize = function(blob) {
  ...
  Yams.prototype.deserialize = function(blob) {
    if (this.options.serialize) {
        ...
        if (err) {
          ...
          if (session) {
            if (!session.expires || new Date() < session.expires) {
              callback(null, self.deserialize(session.session));
            } else {
  ...
  Yams.prototype.set = function(sid, session, callback) {
    ...
    this._get_collection(function(collection) {
      collection.updateOne({_id: sid}, doc, { upsert: true, safe: true }, function(err) {
        if (err) {
          callback(err);
        } else {
          callback(null);
  ...
  Yams.prototype.destroy = function(sid, callback) {
    callback = callback || function(){};
    this._get_collection(function(collection) {
      collection.deleteOne({_id: sid}, function() {
        callback();
      });
    });
  };
